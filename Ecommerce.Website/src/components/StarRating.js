...
const StarRating = (props) => {
	...
	return (
		<div className='flex items-center'>
			{Array.from(Array(Math.floor(star)).keys()).map((item, index) => (
				<svg
					key={index} 
					className={`w-[${size}px] h-[${size}px] text-yellow-400 align-middle`}
					fill="currentColor" 
					viewBox="0 0 20 20" 
			...
			{!Number.isInteger(star) && (
				<svg 
					className={`w-[${size}px] h-[${size}px] text-yellow-400`} 
					fill="currentColor" 
					viewBox="0 0 20 20" 
					xmlns="http://www.w3.org/2000/svg"
				>
			...
			{Array.from(Array(Math.floor(5-star)).keys()).map((item, index) => (
				<svg 
					key={index}
					className={`w-[${size}px] h-[${size}px] text-yellow-400`} 
					fill="currentColor" 
